...
def get_args():
    ...
    xargparser.add_argument('--model_prefix', type=str,
                            default=os.path.join(ROOT, 'weights/yolov5n-seg-smoke'))
...
def main():
    ...
    ANCHORS = np.array(get_anchors(), dtype=np.float32).reshape(3, -1, 2)  # [3, 3, 2]
    STRIDES = [8, 16, 32]
    ...
    for anchor, stride in zip(ANCHORS, STRIDES):
        s = INPUT_SIZE // stride
        yv, xv = np.meshgrid(range(s), range(s), indexing='ij')
        # [S, S, 2] -> [1, S * S, 2]
        grid = np.float32(np.stack((xv, yv), axis=2).reshape(1, -1, 2)) - 0.5
        GRIDS.append(grid)
        ANCHOR_GRIDS.append(anchor.reshape(3, 1, 2))
    ...
    while True:
        if not pause:
            ...
            _, proto = ex.extract(out_names[0])
            _, pred0 = ex.extract(out_names[1])
            _, pred1 = ex.extract(out_names[2])
            _, pred2 = ex.extract(out_names[3])

            proto, pred0, pred1, pred2 = map(np.array, (proto, pred0, pred1, pred2))
            outputs = []
            for i, pred in enumerate((pred0, pred1, pred2)):
                cxcy = (sigmoid(pred[..., 0:2]) * 2 + GRIDS[i]) * STRIDES[i]
                wh = np.power(sigmoid(pred[..., 2:4]) * 2, 2) * ANCHOR_GRIDS[i]
                conf = sigmoid(pred[..., 4:5 + NUM_CLASSES])
                mask = pred[..., 5 + NUM_CLASSES:]
                curr_output = np.concatenate((cxcy, wh, conf, mask), axis=-1)
                curr_output = curr_output.reshape(-1, pred.shape[-1])
                outputs.append(curr_output)
            outputs = np.concatenate(outputs, 0)

            obj_dets = NMS(outputs, CONF_THRESHOLD, IOU_THRESHOLD, nm=args.nm, top_k=MAX_DETS)
            toc = time.time()
