...
def get_args():
    ...
    xargparser.add_argument('--weights', type=str,
                            default=os.path.join(ROOT, 'runs_old/train-seg/coco-smoke-5s-seg/weights/best.pt'))
    ...
    args.model = attempt_load(args.weights, device=DEVICE)
    return args
...
def main():
    ...
    while True:
        if not pause:
            ...
            imgblob = np.transpose(np.float32(img_pad[None, ..., ::-1]) / 255., (0, 3, 1, 2))
            img_tensor = torch.from_numpy(imgblob).to(DEVICE)
            with torch.no_grad():
                pred, proto, _ = model(img_tensor)
            # nc = 80
            # pred: (1, 25200, 117)
            # proto: (1, 32, 160, 160)
